...
using Lean.Identity;
using Lean.Web.Chat.SignalR;
using Lean.Web.Common;
using Swashbuckle.AspNetCore.Swagger;
using Lean.Web.IdentityServer;
using Lean.Web.Swagger;
using Stripe;
    ...
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerDoc("v1", new OpenApiInfo() {Title = "Lean API", Version = "v1"});
            ...
            {
                //var viewAssembly = typeof(EmailBodyRenderer).GetTypeInfo().Assembly;
                //var fileProvider = new EmbeddedFileProvider(viewAssembly);
                //options.FileProviders.Add(fileProvider);
                options.ViewLocationFormats.Add("~/Views/Email/{0}" + RazorViewEngine.ViewExtension);
                options.ViewLocationFormats.Add("~/Views/Email/Shared/{0}" + RazorViewEngine.ViewExtension);
            });

            //services.AddControllersWithViews()
            //    .AddRazorRuntimeCompilation(options => options.FileProviders.Add(
            //        new EmbeddedFileProvider(typeof(EmailBodyRenderer).GetTypeInfo().Assembly)));

            //Configure Abp and Dependency Injection
            return services.AddAbp<LeanWebHostModule>(options =>
            {
